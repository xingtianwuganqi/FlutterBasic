...
class BaseTabBarState extends State<BaseTabBar> {
  ...
  void initState() {
    ...
    for (int i = 0; i<widget.items.length; i++) {
      var currentItem = widget.items[i];
      AssetImage image = AssetImage(currentItem.selectIcon);
      AssetImage unImage = AssetImage(currentItem.unSelectIcon);
      currentItem.selectImg = image;
      currentItem.unSelectImg = unImage;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i<widget.items.length;i++) {
      ...
      if (current.selectImg != null){
        precacheImage(current.selectImg!, context);
      }
      if (current.unSelectImg != null) {
        precacheImage(current.unSelectImg!, context);
      }
    }
  ...
  Widget bottomTabBars() {
    ...
    return BottomNavigationBar(
      ...
      unselectedItemColor: ColorsUtil.hexColor('#515151'),
      selectedFontSize: 12,
  ...
  Widget unreadSelectIcon(bool isSelect, BottomNavigationModel model) {
    return Stack(
      children: [
        isSelect ?
        Image(width: 24, height: 24, image: model.selectImg ?? AssetImage(model.selectIcon))
        : Image( width: 24, height: 24, image: model.unSelectImg ?? AssetImage(model.unSelectIcon)),
        Positioned(
          top: 0,
          right: 0,
